...
//   Calendar, Heart, Camera, Video, Play, ShieldAlert 
// } from "lucide-react";
...
//   const bio = panchayat?.bio || "Gaon ki unnati hamari prathamikta hai.";
...
//         });
//         if (res.ok) {
//             alert("Sandesh bhej diya gaya hai!");
...
//   const infoSections = [
//     { id: "history", title: "History", icon: <Landmark size={20}/>, text: panchayat.history, img: panchayat.config?.historyImg, sub: "Humari Virasat" },
...
//       {/* 🔴 GLOBAL ALERT TICKER */}
//       {globalAlert && (
//           <div className="bg-red-600 text-white py-2 overflow-hidden sticky top-0 z-[1000]">
//               <div className="flex whitespace-nowrap animate-marquee font-black uppercase text-[10px] tracking-[4px]">
//                   {[1,2,3,4,5,6].map(i => <span key={i} className="mx-10 flex items-center gap-2"><ShieldAlert size={14}/> {globalAlert}</span>)}
//               </div>
//           </div>
//       )}

//       {/* 1. NAVBAR (SPACING FIX) */}
//       <nav className={`fixed ${!scrolled && globalAlert ? 'top-8' : 'top-0'} w-full z-[100] transition-all duration-300 ${scrolled ? 'bg-white shadow-lg py-1.5' : 'bg-white py-3 border-b border-slate-100'}`}>
//         <div className="max-w-7xl mx-auto px-4 flex justify-between items-center gap-4">
//             <div className="flex items-center gap-3">
//                 <button onClick={() => setIsMenuOpen(true)} className="lg:hidden p-1 text-slate-900"><Menu size={26} /></button>
//                 <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-full border-2 overflow-hidden bg-slate-50 shadow-inner" style={{ borderColor: themeColor }}>
//                     <img src={avatar} style={{ objectFit: "cover", objectPosition: `center ${avatarPos}%`, transform: `scale(${avatarZoom})`, width: '100%', height: '100%' }} alt="L" />
//                 </div>
//                 <div className="flex flex-col">
//                     <h1 className="text-sm sm:text-lg font-black uppercase leading-tight tracking-tighter">{name}</h1>
//                     <p className="text-[8px] sm:text-[9px] font-bold text-slate-400 uppercase tracking-widest">{leaderName}</p>
//                 </div>
//             </div>
//             <div className="hidden lg:flex items-center gap-8 text-[11px] font-black uppercase tracking-widest text-slate-500">
//                 {['vision', 'history', 'geography', 'economy', 'news'].map(id => <a key={id} href={`#${id}`} className="hover:text-blue-600">{id}</a>)}
//             </div>
//             <button onClick={() => setIsModalOpen(true)} style={{ backgroundColor: themeColor }} className="px-5 py-2 text-white text-[10px] font-black uppercase rounded-full shadow-lg hover:scale-105 transition-all">Support</button>
//         </div>
//       </nav>
...
//               <div className="flex-1 py-4 px-8 flex flex-col gap-1 overflow-y-auto">
//                  {['vision', 'history', 'geography', 'economy', 'news'].map(id => (<a key={id} href={`#${id}`} onClick={() => setIsMenuOpen(false)} className="text-xs font-black uppercase border-b border-slate-50 py-5 text-slate-700 hover:text-blue-600 transition-all">{id}</a>))}
//               </div>
//             </motion.div>
//           </>
//         )}
...
//       {/* 2. HERO SECTION (REDUCED PADDING) */}
//       <section id="vision" className="pt-32 pb-12 px-4 bg-slate-50 overflow-hidden sm:pt-40 sm:pb-20">
...
//             <h1 className="text-4xl sm:text-7xl font-black text-slate-900 leading-[1.1] mb-6 uppercase tracking-tighter">Sashakt Gram, <br/> <span style={{ color: themeColor }}>Naya Sankalp.</span></h1>
//             <p className="text-lg text-slate-600 leading-relaxed mb-8 border-l-4 pl-6 italic max-w-xl mx-auto lg:mx-0 break-words" style={{ borderColor: themeColor, whiteSpace: 'pre-wrap' }}>{bio}</p>
...
//                <a href="#news" className="px-10 py-4 border-2 border-slate-200 font-black text-[10px] uppercase tracking-widest rounded-xl text-slate-700 hover:bg-white transition-all">Latest News</a>
//             </div>
//           </motion.div>
//           <img src={panchayat.config?.banner || "https://images.unsplash.com/photo-1523348837708-15d4a09cfac2"} className="w-full h-[280px] sm:h-[500px] object-cover rounded-[3rem] shadow-2xl" />
//         </div>
//       </section>

//       {/* 3. MANIFESTO (COMPACT) */}
//       <section className="py-16 sm:py-20 px-4 bg-slate-900 text-white rounded-[4rem] mx-4">
//           <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-12 lg:gap-20 items-center text-center lg:text-left">
//               <img src={panchayat.config?.manifestoImg || "https://images.unsplash.com/photo-1517245386807-bb43f82c33c4"} className="w-full h-[300px] sm:h-[500px] object-cover rounded-[3rem] shadow-2xl shadow-black/50" />
//               <div>
//                   <h2 className="text-[10px] font-black uppercase tracking-[10px] text-blue-400 mb-6">Mera Manifesto</h2>
//                   <div className="relative p-8 sm:p-10 bg-white/5 border-l-8 rounded-r-3xl shadow-xl" style={{ borderColor: themeColor }}>
//                       <Quote className="opacity-20 mb-4 sm:mb-6 mx-auto lg:mx-0" size={40} style={{ color: themeColor }} />
...
//       {/* 4. INFORMATION SECTIONS (COMPACT SPACING) */}
//       <div className="space-y-4">
//         {infoSections.map((sec, i) => (
//             <section key={sec.id} id={sec.id} className={`py-16 sm:py-20 px-4 scroll-mt-24 ${i % 2 !== 0 ? 'bg-slate-50' : 'bg-white'}`}>
//             <div className={`max-w-7xl mx-auto grid lg:grid-cols-2 gap-12 lg:gap-20 items-center ${i % 2 !== 0 ? 'lg:flex-row-reverse' : ''}`}>
//                 <motion.div initial={{ opacity: 0, x: i % 2 === 0 ? -30 : 30 }} whileInView={{ opacity: 1, x: 0 }} className="text-center lg:text-left">
...
//                     </div>
//                     <h3 className="text-4xl sm:text-5xl font-black text-slate-900 mb-6 sm:mb-8 uppercase tracking-tighter italic leading-none">{sec.sub}</h3>
//                     <p className="text-lg text-slate-600 leading-relaxed border-l-4 pl-8 mx-auto lg:mx-0 break-words" style={{ borderColor: themeColor, whiteSpace: 'pre-wrap' }}>{sec.text || "Jankari jald hi update ki jayegi."}</p>
//                 </motion.div>
...
//       {/* 5. VIDEO HUB */}
//       {videos.length > 0 && (
//         <section id="videos" className="py-20 px-4 bg-slate-950 text-white rounded-[5rem] mx-4 mb-20 scroll-mt-20">
//           <div className="max-w-7xl mx-auto">
//             <h2 className="text-4xl sm:text-5xl font-black uppercase mb-16 text-center italic tracking-tighter text-white/90">Video Feed</h2>
...
//       {/* 6. GALLERY (EVEN GRID) */}
//       <section id="gallery" className="py-20 px-4 bg-slate-50 scroll-mt-24 border-t">
//         <div className="max-w-7xl mx-auto">
//           <h2 className="text-4xl sm:text-5xl font-black text-slate-900 uppercase mb-16 text-center tracking-tighter">Vikas Gallery</h2>
//           <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
//             {gallery.map((img: string, i: number) => (
//               <div key={i} className="relative aspect-video rounded-[2rem] overflow-hidden shadow-xl bg-slate-200">
//                 <img src={img} className="w-full h-full object-cover hover:scale-105 transition-all duration-700" alt="Work" />
//               </div>
//             ))}
//           </div>
//         </div>
...
//       {/* 7. NEWS UPDATES */}
//       <section id="news" className="py-20 px-4 bg-white scroll-mt-24">
//         <div className="max-w-7xl mx-auto">
//           <h2 className="text-4xl sm:text-5xl font-black text-slate-900 uppercase mb-16 underline decoration-8 decoration-blue-50 underline-offset-8 text-center" style={{ textDecorationColor: `${themeColor}20` }}>Rajasthan News</h2>
//           <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10 text-left">
//             {news?.map((item: any) => (
//               <div key={item.id} className="bg-slate-50 rounded-[3rem] border border-slate-100 p-8 shadow-sm hover:shadow-xl transition-all">
//                 <img src={item._embedded?.['wp:featuredmedia']?.[0]?.source_url || "https://via.placeholder.com/600x400"} className="w-full h-52 object-cover rounded-[2rem] mb-6 grayscale hover:grayscale-0 transition-all duration-500" alt="N" />
...
//                 <h4 className="font-bold text-lg mb-8 line-clamp-2 uppercase leading-tight text-slate-900" dangerouslySetInnerHTML={{ __html: item.title.rendered }}></h4>
//                 <a href={item.link} target="_blank" className="text-[10px] font-black uppercase tracking-[3px] flex items-center gap-2 py-4 px-8 border-2 rounded-2xl hover:bg-slate-900 hover:text-white transition-all">Full Story <ChevronRight size={14}/></a>
//               </div>
//             ))}
//           </div>
//         </div>
...
//       {/* 8. FOOTER */}
//       <footer className="py-20 bg-slate-900 text-white text-center rounded-t-[5rem]">
//         <Heart className="mx-auto mb-6 text-red-500" fill="currentColor" size={32}/>
//         <h3 className="text-3xl font-black uppercase mb-4 tracking-widest">{name}</h3>
//         <p className="text-[10px] text-slate-500 font-bold uppercase tracking-[10px]">RAJGRAM ENGINE © 2026</p>
//       </footer>
...
//                 <div className="flex justify-between items-center mb-10 border-b pb-4"><h2 className="text-2xl font-black uppercase tracking-tight italic text-slate-900">Jan-Sampark</h2><button onClick={()=>setIsModalOpen(false)}><X size={28}/></button></div>
//                 <form onSubmit={handleSupportSubmit} className="space-y-6">
//                     <input required type="text" placeholder="Naam" value={formData.name} onChange={(e)=>setFormData({...formData, name: e.target.value})} className="w-full p-5 border-2 border-slate-100 rounded-2xl outline-none font-bold focus:border-blue-500 text-slate-900" />
//                     <input required type="tel" placeholder="Mobile" value={formData.phone} onChange={(e)=>setFormData({...formData, phone: e.target.value})} className="w-full p-5 border-2 border-slate-100 rounded-2xl outline-none font-bold focus:border-blue-500 text-slate-900" />
//                     <textarea required placeholder="Sandesh..." value={formData.message} onChange={(e)=>setFormData({...formData, message: e.target.value})} className="w-full p-5 border-2 border-slate-100 rounded-2xl h-32 font-bold resize-none focus:border-blue-500 text-slate-900" />
//                     <button type="submit" style={{ backgroundColor: themeColor }} className="w-full py-6 text-white font-black uppercase rounded-2xl shadow-xl hover:brightness-110">Submit samarthan</button>
//                 </form>
//             </motion.div>
//           </div>
//         )}
  ...
  if (panchayat.status === "suspended") {
    return (
        <div className="min-h-screen bg-[#020617] flex items-center justify-center p-6 text-center">
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="max-w-md w-full bg-slate-900 border-2 border-red-500/30 p-12 rounded-[3rem] shadow-2xl">
                ...
                <p className="text-slate-400 font-bold text-sm uppercase tracking-widest leading-relaxed mb-10">Is Gram Panchayat ki digital sevaayein temporary rok di gayi hain. <br/> <span className="text-red-400 mt-2 block italic font-black">Kripya Admin se sampark karein.</span></p>
                <div className="pt-8 border-t border-white/5 text-[10px] font-black uppercase text-slate-600 tracking-[5px]">RAJGRAM SECURITY PROTOCOL</div>
            </motion.div>
        </div>
    );
  }
