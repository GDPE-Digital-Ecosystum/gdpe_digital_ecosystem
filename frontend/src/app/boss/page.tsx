...
//   Heart, Trash2, CheckCircle2, ChevronDown, Rocket, Database 
// } from "lucide-react";
...
//     const loadData = useCallback(async (q = "", p = 0) => {
//         setLoading(true);
//         try {
//             const res = await fetch(`/api/boss/advanced?search=${q}&page=${p}`);
...
//     useEffect(() => {
//         if (!isMounted) return;
//         const timer = setTimeout(() => { loadData(search, page); }, 600);
//         return () => clearTimeout(timer);
//     }, [search, page, isMounted, loadData]);
...
//             body: JSON.stringify({ action, data: { slug: data, ...newNeta, alert: alertText } }) 
//         });
//         if(res.ok) { loadData(search, page); setIsCreateOpen(false); }
//     };
...
//     return (
//         <div className="min-h-screen bg-[#020617] text-white flex font-sans overflow-hidden text-slate-800">
            
//             {/* --- 1. SIDEBAR --- */}
//             <div className="w-20 lg:w-80 bg-slate-900/50 border-r border-white/5 p-8 flex flex-col h-screen sticky top-0 shrink-0">
//                 <div className="mb-10 text-center">
//                     <div className="w-12 h-12 bg-blue-600 rounded-2xl flex items-center justify-center mx-auto mb-4 shadow-lg"><Rocket size={24} className="text-white"/></div>
//                     <h2 className="hidden lg:block text-2xl font-black italic text-blue-500 uppercase tracking-tighter">BOSSHUB</h2>
//                 </div>
...
//                     ].map(t => (
//                         <button key={t.id} onClick={()=>setActiveTab(t.id)} className={`w-full flex items-center justify-center lg:justify-start gap-4 p-4 rounded-xl font-bold uppercase text-[10px] tracking-widest transition-all ${activeTab===t.id?'bg-blue-600 shadow-lg text-white':'text-slate-500 hover:bg-slate-800'}`}>
//                             {t.icon} <span className="hidden lg:block uppercase">{t.label}</span>
//                         </button>
//                     ))}
//                 </nav>
//                 <button onClick={() => signOut({ callbackUrl: "/login" })} className="p-4 bg-red-600/10 text-red-500 rounded-xl hover:bg-red-600 hover:text-white transition-all flex justify-center"><LogOut size={18}/></button>
//             </div>

//             {/* --- 2. MAIN AREA --- */}
//             <div className="flex-1 p-6 lg:p-12 overflow-y-auto">
//                 <div className="flex justify-between items-center mb-10">
//                     <button onClick={() => loadData(search, page)} className="flex items-center gap-2 bg-slate-900 border border-white/5 px-6 py-3 rounded-2xl hover:bg-blue-600 transition-all shadow-xl">
...
//                     </button>
//                     <div className="text-right hidden sm:block"><p className="text-[10px] text-slate-500 font-bold uppercase tracking-[4px]">Rajasthan Master Control</p></div>
//                 </div>

//                 <AnimatePresence mode="wait">
//                     {/* ✅ TAB: MANAGEMENT (LIST) */}
//                     {activeTab === 'overview' && (
//                         <motion.div key="ov" initial={{opacity:0, y:10}} animate={{opacity:1, y:0}} exit={{opacity:0}} className="space-y-12">
//                             <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
//                                 <div className="bg-blue-600 p-8 rounded-[3rem] shadow-2xl text-center"><p className="text-[10px] font-black uppercase opacity-60 mb-2">Total Sites</p><h2 className="text-6xl font-black italic">{stats?.totalGPs || 11075}</h2></div>
//                                 <div className="bg-slate-900/40 p-8 rounded-[3rem] border border-white/5 text-center"><p className="text-[10px] font-black uppercase text-slate-500 mb-2">Active</p><h2 className="text-6xl font-black text-green-500 italic">{stats?.activeLeadersCount || 0}</h2></div>
//                                 <div className="bg-slate-900/40 p-8 rounded-[3rem] border border-white/5 text-center"><p className="text-[10px] font-black uppercase text-slate-500 mb-2">Voters</p><h2 className="text-6xl font-black text-orange-500 italic">{stats?.totalVotersCount || 0}</h2></div>
//                             </div>

//                             <div className="flex flex-col lg:flex-row gap-4 items-center">
//                                 <div className="relative flex-1 group"><Search className="absolute left-5 top-5 text-slate-500"/><input type="text" placeholder="Search..." className="w-full bg-slate-900 border border-white/10 p-5 pl-14 rounded-2xl font-bold outline-none focus:border-blue-500 text-white" value={search} onChange={(e)=>setSearch(e.target.value)} /></div>
...
//                                         <tr><th className="p-8">Panchayat Name</th><th>Status</th><th className="p-8 text-right">Emergency Console</th></tr>
//                                     </thead>
//                                     <tbody className="divide-y divide-white/5">
//                                         {loading && (!stats?.panchayatList || stats.panchayatList.length === 0) ? (
//                                             <tr><td colSpan={3} className="p-20 text-center text-blue-500 font-black animate-pulse uppercase">Syncing Data...</td></tr>
//                                         ) : (
...
//                                                     <td className="text-center"><span className={`px-4 py-1 rounded-full text-[9px] font-black uppercase border ${gp.status==='active'?'text-green-500 border-green-500/20':gp.status==='suspended'?'text-red-500 border-red-500/20':'text-slate-600 border-slate-700'}`}>{gp.status}</span></td>
//                                                     <td className="p-8 flex justify-end gap-3 opacity-60 group-hover:opacity-100 transition-all">
...
//                                                                 ) : (
//                                                                     <button onClick={()=>runAction("reactivate_leader", gp.slug)} className="p-3 bg-green-950 text-green-500 rounded-xl"><CheckCircle2 size={14}/></button>
//                                                                 )}
//                                                             </div>
//                                                         ) : null}
...
//                     {/* ✅ TAB: DISTRICT STATS (ANALYTICS) */}
//                     {activeTab === 'analytics' && (
//                         <motion.div key="an" initial={{opacity:0}} animate={{opacity:1}} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 text-slate-300">
...
//                                     <div className="flex justify-between items-end mb-4 font-mono"><p className="text-xs font-bold uppercase">{d.active} of {d.total}</p><p className="text-3xl font-black text-blue-500">{Math.round((d.active/d.total)*100)}%</p></div>
...
//                     {/* ✅ TAB: GLOBAL VOTERS (LEADS) */}
//                     {activeTab === 'leads' && (
//                         <motion.div key="ld" initial={{opacity:0}} animate={{opacity:1}} className="bg-slate-900/20 rounded-[3.5rem] border border-white/5 overflow-hidden shadow-2xl text-slate-300">
//                              <table className="w-full text-left">
...
//                                             <td className="text-slate-600 text-xs text-right pr-10">{formatDate(l.date)}</td>
//                                         </tr>
//                                     ))}
//                                 </tbody>
//                             </table>
...
//                             <Bell className="mx-auto text-red-500 mb-6" size={40}/><h3 className="text-2xl font-black mb-10 uppercase text-white tracking-tighter italic">Global Alert Center</h3>
//                             <textarea value={alertText} onChange={(e)=>setAlertText(e.target.value)} className="w-full bg-black border-2 border-white/10 p-10 rounded-[3.5rem] mb-10 outline-none text-white text-xl font-bold focus:border-red-500" placeholder="Type notification for all sites..." />
//                             <button onClick={()=>runAction("update_alert", "")} className="w-full py-6 bg-red-600 rounded-3xl font-black uppercase text-xs tracking-widest hover:bg-red-700 transition-all">Broadcast Now</button>
//                         </motion.div>
//                     )}
//                 </AnimatePresence>
//             </div>
...
//                         <motion.div initial={{scale:0.9, opacity:0}} animate={{scale:1, opacity:1}} exit={{scale:0.9, opacity:0}} className="relative w-full max-w-md bg-[#0f172a] p-10 rounded-[4rem] border border-white/10 shadow-2xl text-slate-800">
...
//                                 <select required className="w-full p-4 bg-black border border-white/10 rounded-2xl outline-none focus:border-blue-500 text-white font-bold text-sm" value={newNeta.slug} onChange={(e)=>setNewNeta({...newNeta, slug: e.target.value})}>
...
//                                 </select>
//                                 <input required placeholder="Leader Name" className="w-full p-4 bg-black border border-white/10 rounded-2xl outline-none focus:border-blue-500 text-white font-bold text-sm" onChange={(e)=>setNewNeta({...newNeta, leaderName: e.target.value})} />
...
//                                 <input required type="password" placeholder="Password" className="w-full p-4 bg-black border border-white/10 rounded-2xl outline-none focus:border-blue-500 text-white font-bold text-sm" onChange={(e)=>setNewNeta({...newNeta, password: e.target.value})} />
//                                 <button className="w-full py-6 bg-blue-600 text-white rounded-3xl font-black uppercase text-xs tracking-widest hover:bg-blue-700 transition-all">Verify & Activate</button>
//                             </form>
//                         </motion.div>
//                     </div>
//                 )}
