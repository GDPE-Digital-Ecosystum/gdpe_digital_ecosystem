    ...
    const renderEditor = (label: string, text: string, setText: any, img: string, setImg: any, type: string) => (
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6 w-full max-w-4xl">
            <div className="bg-white p-6 md:p-10 rounded-[2rem] shadow-xl border">
                ...
                <div className="flex flex-col sm:flex-row gap-6">
                    <label className="flex-1 border-2 border-dashed rounded-[2rem] p-10 cursor-pointer flex flex-col items-center justify-center hover:bg-slate-50 transition-all text-center">
                        {uploading === type ? <Loader2 className="animate-spin text-blue-600"/> : <UploadCloud className="text-blue-600 mb-2"/>}
                        <span className="text-[10px] font-black uppercase text-slate-500">Upload Image</span>
                        <input type="file" className="hidden" onChange={(e)=>e.target.files && uploadImg(e.target.files[0], setImg, type)} />
                    </label>
            ...
            <div className="bg-white p-6 md:p-10 rounded-[2rem] shadow-xl border text-slate-800">
                <h3 className="text-lg md:text-xl font-black mb-6 uppercase italic tracking-tighter underline underline-offset-8 decoration-blue-100">{label} Content</h3>
                <textarea value={text} onChange={(e)=>setText(e.target.value.substring(0, 800))} className="w-full p-6 border-2 border-slate-50 rounded-2xl h-48 md:h-64 text-base bg-slate-50/30 outline-none focus:border-blue-600" />
            </div>
        </motion.div>
    );

    if (fetching) return <div className="h-screen flex items-center justify-center font-black text-blue-600 animate-pulse tracking-[10px]">RAJGRAM SYSTEM...</div>;

    return (
        <div className="min-h-screen bg-[#f8fafc] flex flex-col lg:flex-row font-sans overflow-x-hidden">
            
            {/* --- 📱 MOBILE TOP NAV --- */}
            <div className="lg:hidden bg-[#0f172a] text-white p-4 flex justify-between items-center sticky top-0 z-[100] shadow-2xl">
                <h2 className="text-xl font-black text-blue-500 italic uppercase">RAJGRAM</h2>
                <button onClick={() => setIsSidebarOpen(true)} className="p-2 bg-slate-800 rounded-xl">
                    <Menu size={24} />
                </button>
            </div>

            {/* --- 📁 SIDEBAR (Laptop Persistent / Mobile Drawer) --- */}
            <AnimatePresence>
                {(isSidebarOpen || (typeof window !== 'undefined' && window.innerWidth >= 1024)) && (
                    <>
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setIsSidebarOpen(false)} className="fixed inset-0 bg-black/70 backdrop-blur-sm z-[110] lg:hidden" />
                        <motion.div 
                            initial={typeof window !== 'undefined' && window.innerWidth < 1024 ? { x: "-100%" } : {}}
                            animate={{ x: 0 }}
                            exit={typeof window !== 'undefined' && window.innerWidth < 1024 ? { x: "-100%" } : {}}
                            transition={{ type: "spring", damping: 25, stiffness: 200 }}
                            className="fixed lg:sticky top-0 left-0 bottom-0 w-72 md:w-80 bg-[#0f172a] text-white p-8 flex flex-col z-[120] lg:z-0 shadow-2xl h-screen overflow-y-auto"
                        >
                            <h2 className="text-3xl font-black text-blue-500 italic uppercase mb-12 hidden lg:block tracking-tighter">RAJGRAM</h2>
                            <nav className="flex-1 space-y-1">
                                {[
                                    {id:'analytics', label:'Overview', icon:<TrendingUp size={16}/>},
                                    {id:'branding', label:'Identity', icon:<Palette size={16}/>},
                ...
                <AnimatePresence mode="wait">
                    ...
                    {activeTab === 'analytics' && (
                        <motion.div key="analytics" initial={{opacity:0, y:10}} animate={{opacity:1, y:0}} exit={{opacity:0}} className="space-y-8">
                            ...
                            <div className="bg-white p-10 md:p-12 rounded-[3rem] border shadow-xl text-center">
                                <Sparkles className="mx-auto text-blue-600 mb-6 animate-pulse" size={48} />
                                <h3 className="text-xl md:text-2xl font-black uppercase italic tracking-tighter text-slate-800">Your Portal is Live</h3>
                                <p className="text-slate-500 mt-4 max-w-lg mx-auto text-[10px] font-bold uppercase tracking-widest leading-relaxed">Update your branding and content regularly.</p>
                            </div>
                        </motion.div>
                    )}
                    ...
                    {activeTab === 'branding' && (
                        <motion.div key="branding" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-8 max-w-4xl w-full">
                            <div className="bg-white p-6 md:p-10 rounded-[2.5rem] shadow-xl border">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                                    <div className="space-y-6 text-slate-800">
                                        <input type="text" value={leaderName} onChange={(e)=>setLeaderName(e.target.value)} className="w-full p-4 border-2 border-slate-50 rounded-xl font-bold bg-slate-50 focus:border-blue-600 outline-none" placeholder="Full Name" />
                                        <label className="border-2 border-dashed rounded-[2rem] p-8 cursor-pointer flex flex-col items-center justify-center hover:bg-slate-50 transition-all text-center">
                                            {uploading==='avatar'? <Loader2 className="animate-spin text-blue-600"/> : <UploadCloud className="text-blue-600 mb-2"/>}
                            ...
                            <div className="bg-white p-6 md:p-10 rounded-[2.5rem] shadow-xl border">
                                <h3 className="text-lg font-black uppercase mb-6 flex gap-3 text-slate-800"><ImageIcon/> Hero Banner</h3>
                                <label className="border-2 border-dashed rounded-[2.5rem] p-12 md:p-20 cursor-pointer flex flex-col items-center mb-6 hover:bg-slate-50 transition-all text-center">
                                    {uploading === 'banner' ? <Loader2 className="animate-spin text-blue-600"/> : <UploadCloud className="text-blue-600"/>}
                                    <span className="text-[10px] font-black mt-2 text-slate-500 uppercase tracking-widest text-slate-800">Select Banner Photo</span>
                                    <input type="file" className="hidden" onChange={(e) => e.target.files && uploadImg(e.target.files[0], setBanner, 'banner')} />
                                </label>
                                {banner && <img src={banner} className="h-48 md:h-64 w-full object-cover rounded-3xl border shadow-lg" alt="B" />}
                            </div>
                    ...
                    {activeTab === 'gallery' && (
                        <div className="bg-white p-6 md:p-10 rounded-[2.5rem] shadow-xl border max-w-4xl w-full text-slate-800">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-8">
                                {gallery.map((url, i) => (
                                    <div key={i} className="space-y-4 text-slate-800">
                                        <label className="border-2 border-dashed rounded-2xl p-6 cursor-pointer flex justify-between items-center hover:bg-slate-50 transition-all">
                                            <span className="text-[10px] font-black uppercase tracking-widest text-slate-500 italic">Photo #{i+1}</span>
                                            {uploading===`gal-${i}` ? <Loader2 className="animate-spin text-blue-600"/> : <UploadCloud size={18} className="text-blue-600"/>}
                                            <input type="file" className="hidden" onChange={(e) => { if(e.target.files) { uploadImg(e.target.files[0], (link:string)=>{const n=[...gallery];n[i]=link;setGallery(n);}, `gal-${i}`); } }} />
                                        </label>
                                        {url && <img src={url} className="h-40 md:h-48 w-full object-cover rounded-2xl border shadow-xl transition-transform hover:scale-[1.02]" alt="G" />}
                    ...
                    {activeTab === 'videos' && (
                        <div className="bg-white p-6 md:p-10 rounded-[3rem] shadow-xl border max-w-2xl w-full text-slate-800 text-center">
                            <h3 className="text-lg md:text-2xl font-black uppercase mb-10 flex gap-3 justify-center italic tracking-tighter text-slate-800"><Video className="text-red-600" size={32}/> Youtube Feed</h3>
                            <div className="space-y-6 text-left">
                                {videos.map((url, i) => (
                                    <div key={i}>
                                        <label className="text-[9px] font-black uppercase text-slate-400 mb-2 ml-2 block italic tracking-widest">Video Link #{i+1}</label>
                                        <input type="text" value={url} onChange={(e)=>{const n=[...videos];n[i]=e.target.value;setVideos(n);}} className="w-full p-4 md:p-5 border-2 border-slate-50 rounded-2xl bg-slate-50 font-mono text-xs focus:border-red-600 outline-none shadow-inner text-slate-900" placeholder="Paste link..." />
                                    </div>
                                ))}
                            </div>
                        </div>
                    ...
                    {/* VOTERS */}
                    {activeTab === 'voters' && (
                        <div className="bg-white rounded-[2.5rem] md:rounded-[3.5rem] shadow-2xl border overflow-hidden max-w-5xl w-full text-slate-900">
                             <div className="overflow-x-auto">
                                <table className="w-full text-left border-collapse min-w-[700px]">
                                    <thead className="bg-slate-50 border-b"><tr className="text-[10px] font-black uppercase tracking-widest text-slate-400 italic font-mono leading-none"><th className="p-8">Supporter</th><th className="p-8">Contact</th><th className="p-8">Message</th><th className="p-8 text-right">Date</th></tr></thead>
                                    <tbody className="divide-y divide-slate-100 font-bold">
                                        {voters.map((v: any, i) => (
                                            <tr key={i} className="hover:bg-blue-50 transition-all group text-slate-700">
                                                <td className="p-8 flex items-center gap-4"><div className="w-10 h-10 rounded-full bg-blue-100 text-blue-600 flex items-center justify-center font-black">{v.name[0]}</div>{v.name}</td>
                                                <td className="p-8 font-mono text-blue-600 font-black">{v.phone}</td>
                                                <td className="p-8 text-slate-500 italic text-sm font-medium line-clamp-1 max-w-[200px]">"{v.message}"</td>
                                                <td className="p-8 text-[9px] text-slate-400 uppercase text-right">{new Date(v.date).toLocaleDateString()}</td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>
